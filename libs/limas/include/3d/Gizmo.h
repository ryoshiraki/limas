...
public:
  ...
  Gizmo()
  : window_(nullptr), b_enable_(false), b_interacting_(false) {
  ...
  void enableInput(Window::Ptr& window) {
    ...
    gizmo_rotate_->SetScreenDimension(window_->getWidth(), window_->getHeight());
  ...
  glm::vec3 getTranslation() const {
    return glm::vec3(obj_mat_[3]);
  }
  
  glm::quat getRotation() const {
    return glm::quat(obj_mat_);
  }
  ...
  glm::vec3 getScale() const {
    ...
    glm::decompose(obj_mat_, scale, orientation, translation, skew, perspective);
    return scale;
  }
  ...
  void setScaleAxisMask(unsigned int mask) {
    gizmo_scale_->SetAxisMask(mask);
  }
