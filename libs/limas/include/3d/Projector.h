...
static const string projectorVertexShader =
    gl::vertexShaderHeader +
    STRINGIFY(layout(location = 0) IN vec3 position;
              layout(location = 1) IN vec4 color;
              layout(location = 2) IN vec2 texcoord;
              layout(location = 3) IN vec3 normal;

              OUT vec4 v_color; OUT vec4 v_texcoord;

              uniform mat4 u_view_mat; uniform mat4 u_proj_mat;
...
static const string projectorFragmentShader =
    gl::fragmentShaderHeader +
    STRINGIFY(IN vec4 v_color; IN vec4 v_texcoord;

              OUT vec4 frag_color; uniform sampler2D u_tex;
              uniform mat4 u_tex_mat;
...
class Projector : public Camera {
 public:
  ...
  void bind(const gl::TextureBase& tex, const glm::mat4& view,
            const glm::mat4& proj) {
    ...
    glm::mat4 tex_mat =
        bias_mat * getProjectionMatrix(0, 0, tex.getWidth(), tex.getHeight()) *
        getModelViewMatrix();
    ;
